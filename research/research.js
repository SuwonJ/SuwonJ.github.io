  ...
  try {
    ...
    if (pdfMedia.length > 0) {
      for (const pdfItem of pdfMedia) {
        ...
        await renderPdf(pdfItem.url, pdfWrapper);
      }
    }
...
async function renderPdf(fileUrl, targetContainer) {
  if (typeof pdfjsLib === 'undefined') {
    targetContainer.innerHTML += `<p><a href="${fileUrl}" target="_blank">PDF 문서 보기 ↗</a></p>`;
    return;
  }
  ...
  try {
    ...
    pdfContainer.innerHTML = `<p style="color:#fc5c65;">PDF 렌더링 오류: ${e.message}</p>`;
  }
}
...
async function renderPostList(container) {
  try {
    ...
    const posts = await fetchPostsByTag("research");
